...
import { Assessment, AnalysisMetric, DietPlan, ComparativeChange, ActionPlan } from '../types';
...
// --- Sub-componentes para evitar importações complexas ---

const getStatusClasses = (status: string) => {
  switch (status) {
    case 'bom': return 'bg-green-500 text-green-900';
    case 'atencao': return 'bg-yellow-500 text-yellow-900';
    case 'melhorar': return 'bg-red-500 text-red-900';
...
const MetricCard: React.FC<{ metric: AnalysisMetric }> = ({ metric }) => (
  <div className="bg-gray-700/50 p-4 rounded-lg break-inside-avoid">
    ...
    <p className="text-sm text-gray-400 mt-2"><strong>Ideal:</strong> {metric.idealRange}</p>
...
const AnalysisSection: React.FC<{ title: string; items: string[]; icon: React.ReactNode; iconBgColor: string }> = ({ title, items, icon, iconBgColor }) => (
  <div className="bg-gray-700/50 p-5 rounded-lg break-inside-avoid">
    <h4 className="font-bold text-white flex items-center mb-3">
      <span className={`w-8 h-8 rounded-full flex items-center justify-center mr-3 ${iconBgColor}`}>{icon}</span>
      {title}
    </h4>
...
const PrintableSingleReport = forwardRef<HTMLDivElement, PrintableSingleReportProps>(({ assessment }, ref) => {
  return (
    <div ref={ref} className="bg-gray-900 text-gray-300" style={{ width: '1124px', padding: '40px' }}>
      <div className="printable-card-wrapper p-8 bg-gray-800 rounded-xl">
...
export default PrintableSingleReport;